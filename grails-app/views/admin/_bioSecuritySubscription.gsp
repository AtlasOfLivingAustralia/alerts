<div name="subscription_${query.id}" class="row bioscecrurity-padding" style="background-color: ${(i+startIdx) % 2 == 0 ? '#f0f0f0' : '#ffffff'};">
    <div class="col-md-1">${i+startIdx+1}.

    </div>
    <div class="col-md-4"><g:link controller="query" action="show" id="${query.id}">${query.name}</g:link></div>
    <div class="col-md-4">
    ...
    <div class="col-md-3">
        <form class="form-group" action="${request.contextPath}/admin/testBiosecurity?queryid=${query.id}" method="post" name="previewAndEmail">
                ...
                <button class="btn btn-primary" type="button" onclick="submitPreview('${request.contextPath}/admin/testBiosecurity?queryid=${query.id}', this, true)" >Preview</button>
